...
// NEW: Recipe Management Elements
...
async function showSection(sectionId) {
    ...
    if (sectionId === 'order-management') {
        ...
        await populateRecipeIngredientSelect(); // NEW: Populate ingredients for recipe builder
        renderCurrentRecipe(); // NEW: Render the current menu item's recipe (if editing)
    }
}
...
inventoryForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        await populateRecipeIngredientSelect(); // NEW: Refresh ingredient list for recipe builder
    } catch (error) {
        // Error handled by fetchData
    }
});
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // NEW: Populate recipe ingredient select on load (after inventory is loaded)
