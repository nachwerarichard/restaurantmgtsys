...
const inventoryController = {
    ...
    createIngredient: async (req, res) => {
        const { name, costPerUnit } = req.body;
        const newIngredient = new Ingredient({ name, costPerUnit });
...
const kitchenOrderController = {
    ...
    markOrderReady: async (req, res) => {
        ...
        try {
            ...
            try {
                // Record sales
                await Sale.insertMany(salesToRecord);
                ...
                await createAuditLog('system', 'update_failed', `Failed to process order #${id}. Error: ${error.message}`);
